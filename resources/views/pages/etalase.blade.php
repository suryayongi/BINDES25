    ...
    <div class="container py-5">
        ...
        @forelse ($productsBySeller as $sellerId => $products)
            <div class="seller-section mb-5">
                {{-- Bagian Judul Toko dan Tombol WhatsApp --}}
                ...
                {{-- Grid untuk produk dari penjual ini --}}
                <div class="row row-cols-1 row-cols-sm-2 row-cols-md-3 row-cols-lg-4 g-4">
                    @foreach ($products as $product)
                        <div class="col">
                            <div class="card h-100 shadow-sm border-0">
                                {{-- PERBAIKAN FINAL UNTUK GAMBAR --}}
                                <img src="{{ asset('storage/' . $product->image) }}" class="card-img-top" alt="{{ $product->name }}" style="height: 200px; object-fit: cover;">
                                
                                <div class="card-body d-flex flex-column">
                                    <h5 class="card-title fw-bold" style="color: var(--text-green);">{{ $product->name }}</h5>
                                    <p class="card-text fw-bold fs-5 mb-3">Rp{{ number_format($product->price, 0, ',', '.') }}</p>
                                ...
                                <div class="modal-content">
                                    ...
                                    <div class="modal-body">
                                        <div class="row g-4">
                                            ...
                                            <div class="col-md-6">
                                                <p><strong>Harga:</strong> <span class="fs-4 fw-bold text-success">Rp{{ number_format($product->price, 0, ',', '.') }}</span></p>
